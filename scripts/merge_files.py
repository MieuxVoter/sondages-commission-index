...
def merge_files(base_csv="base.csv", files_csv="files.csv", output_csv="notices_catalog.csv"):
    ...
    merged_df = df_files.merge(df_base[["name", "categorie", "year", "href"]], on="name", how="left")

    # Combine path_local and filename to create full path
    merged_df["pdf_path"] = merged_df.apply(
        lambda row: (
            str(Path(row["path_local"]) / row["filename"])
            if pd.notna(row["path_local"]) and pd.notna(row["filename"])
            else None
        ),
        axis=1,
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Merge base.csv and files.csv into a comprehensive catalog")

    parser.add_argument("--base-csv", default="base.csv", help="Path to base CSV file (default: base.csv)")

    parser.add_argument("--files-csv", default="files.csv", help="Path to files CSV file (default: files.csv)")

    parser.add_argument(
        "--output", default="notices_catalog.csv", help="Output path for merged catalog (default: notices_catalog.csv)"
    )

    args = parser.parse_args()

    merge_files(base_csv=args.base_csv, files_csv=args.files_csv, output_csv=args.output)
